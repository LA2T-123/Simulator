...
function init() {
    ...
    // Set default difficulty to 2 instead of 3
    if (difficultySlider.value === '3') {
        difficultySlider.value = '2';
        difficultyValue.textContent = '2';
    }
...
// Calculate and update estimated time to complete 1 BTC
function updateBTCETA() {
    if (!isMining) return;
    
    const etaEl = document.getElementById('btc-eta');
    const difficulty = parseInt(difficultySlider.value);
    const hashrate = parseFloat(hashrateSlider.value);
    
    // Calculate average time to find a block based on difficulty and hashrate
    // Higher difficulty means more zeros needed, exponentially harder
    const avgHashesPerBlock = 16 ** difficulty; // 16 possible hex chars, need 'difficulty' zeros
    const secondsPerBlock = avgHashesPerBlock / hashrate;
    
    // Calculate how many more blocks needed to reach 1 BTC
    const blockReward = 6.25 / (2 ** Math.floor(minedBlocks / 210000)); // Current block reward
    const btcRemaining = BTC_TARGET - bitcoinEarned;
    const blocksNeeded = Math.ceil(btcRemaining / blockReward);
    
    // Calculate total seconds remaining
    const secondsRemaining = blocksNeeded * secondsPerBlock;
    
    if (secondsRemaining === Infinity || isNaN(secondsRemaining)) {
        etaEl.textContent = 'Est. Time: --:--:--';
        return;
    }
    ...
    etaEl.textContent = `Est. Time: ${timeString}`;
}
